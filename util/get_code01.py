...
class CrackGeetest01():
    ...
    def get_snap(self):
        ...
        # 保存图片
        self.driver.save_screenshot('Image1234567890.png')
        page_snap_obj = Image.open('Image1234567890.png')
    ...
    def crack(self):
        ...
        for track in tracks['forward_tracks']:
            ActionChains(self.driver).move_by_offset(xoffset=track, yoffset=50).perform()
            #print(tracks)

        # 结果傻逼了，正常的人类停顿了一下，回过神来发现，卧槽，滑过了,然后开始反向滑动
        time.sleep(0.6)
        for back_track in tracks['back_tracks']:
            ActionChains(self.driver).move_by_offset(xoffset=back_track, yoffset=50).perform()
            #print(back_track)

        # 小范围震荡一下，进一步迷惑极验后台，这一步可以极大地提高成功率
        time.sleep(0.5)
        ActionChains(self.driver).move_by_offset(xoffset=3, yoffset=50).perform()  # 先移动去一点
        time.sleep(0.6)
        ActionChains(self.driver).move_by_offset(xoffset=-3, yoffset=50).perform()  # 再退回来，模仿人的行为习惯
